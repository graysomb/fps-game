...
// Octree structure for voxel spatial partitioning
...
static OctreeNode *voxelOctreeRoot = NULL;
static bool octreeDirty = true;
...
static OctreeNode *octree_create_node(Vector3 center, float halfSize) {
...
static void octree_free(OctreeNode *node) {
    if (!node) return;
    for (int i = 0; i < 8; i++) {
        if (node->children[i]) octree_free(node->children[i]);
    }
    free(node);
}

static OctreeNode *octree_build_recursive(Vector3 center, float halfSize, const int *indices, int count, int depth) {
    (void)depth; // depth is currently unused but kept for potential debugging/constraints
    OctreeNode *node = octree_create_node(center, halfSize);
    if (!node) return NULL;
    node->voxelCount = count;

    if (count <= 1) {
        node->voxelIndex = (count == 1) ? indices[0] : -1;
        node->isLeaf = true;
        return node;
    }
    ...
    for (int i = 0; i < count; i++) {
        const Voxel *v = &voxels[indices[i]];
        int octant = 0;
        if (v->pos.x >= center.x) octant |= 1;
        if (v->pos.y >= center.y) octant |= 2;
        if (v->pos.z >= center.z) octant |= 4;
        childCounts[octant]++;
    }
    ...
    for (int i = 0; i < 8; i++) {
        offsets[i] = prefix;
        prefix += childCounts[i];
    }

    int *buffer = NULL;
    if (prefix > 0) {
        buffer = (int *)malloc(prefix * sizeof(int));
        if (!buffer) {
            // Allocation failed: collapse into a leaf to avoid losing voxels
            node->isLeaf = true;
            node->voxelIndex = indices[0];
            return node;
        }
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (v->pos.y >= center.y) octant |= 2;
        if (v->pos.z >= center.z) octant |= 4;
        buffer[offsets[octant] + filled[octant]] = indices[i];
        filled[octant]++;
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        childCenter.x += (i & 1) ? childHalf : -childHalf;
        childCenter.y += (i & 2) ? childHalf : -childHalf;
        childCenter.z += (i & 4) ? childHalf : -childHalf;
        node->children[i] = octree_build_recursive(childCenter, childHalf, buffer + offsets[i], childCounts[i], depth + 1);
    }
...
static OctreeNode *build_voxel_octree(void) {
    ...
    for (int i = 0; i < voxel_count; i++) {
        if (voxel_is_active(&voxels[i])) activeCount++;
    }

    if (activeCount == 0) {
        Vector3 center = { 0.0f, 0.0f, 0.0f };
        OctreeNode *root = octree_create_node(center, VOXEL_SIZE * 0.5f);
        if (root) {
            root->voxelIndex = -1;
            root->voxelCount = 0;
            root->isLeaf = true;
    ...
    int *indices = (int *)malloc(activeCount * sizeof(int));
    ...
    maxExtent = fmaxf(maxExtent, VOXEL_SIZE);
    float cubeSize = VOXEL_SIZE;
    while (cubeSize < maxExtent) cubeSize *= 2.0f;
    float halfSize = cubeSize * 0.5f;
    if (halfSize <= 0.0f) halfSize = VOXEL_SIZE * 0.5f;

    OctreeNode *root = octree_build_recursive(center, halfSize, indices, activeCount, 0);
    free(indices);
    return root;
}

static void rebuild_voxel_octree(void) {
    if (voxelOctreeRoot) {
        octree_free(voxelOctreeRoot);
        voxelOctreeRoot = NULL;
    }
