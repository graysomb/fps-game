 ...
 * Gamepad: LS (Move), RS (Look), B (Jump), RT (Shoot), A (Melee), RB (Build), LT (Tether)
...
#define FLUID_DEACTIVATION_VELOCITY_THRESHOLD 0.40f
#define FLUID_DEACTIVATION_FRAMES 8
...
#define AIM_ASSIST_TURN_RATE 5.0f
...
static Particle *particle_create(Vector3 pos, float inv_mass) { //pointer
    ...
    p->collision_group = -1;
    p->glue_count = 0;
    memset(p->jacobi_slots, 0, sizeof(p->jacobi_slots));
...
static void apply_particle_accumulators_range(int start, int end, int worker_id, void *user) {
    ...
    for (int i = start; i < end; ++i) {
        Particle *p = dynamic_particle_at(i);
        if (!p || !p->active || p->inv_mass <= 0.0f) continue;
        Vector3 total_dx = {0, 0, 0};
        float total_weight = 0.0f;
        for (int slot = 0; slot < 8; ++slot) {
            if (p->jacobi_slots[slot].weight > 0.0f) {
                total_dx = v_add(total_dx, p->jacobi_slots[slot].dx);
                total_weight += p->jacobi_slots[slot].weight;
            }
        }
        ...
        if (corr_weight > 0.0f) {
            ...
            total_dx = v_add(total_dx, corr_sum);
        ...
        if (total_weight > 0.0f) {
            Vector3 delta = v_mul(total_dx, PBD_SOR_FACTOR / total_weight);
                              ...
                              int owner)
{
    ...
    for (int i = 0; i < VOXEL_CORNER_COUNT; ++i) {
        ...
        if (!p) {
            for (int j = 0; j < i; ++j) {
                if (v->particles[j]->glue_count > 0) {
                    v->particles[j]->glue_count--;
                }
...
static void break_face_link(Voxel *voxel, int face_index) {
    ...
    int nidx = voxel->glued_neighbors[face_index];
    if (nidx >= 0 && nidx < voxel_count) {
        voxels[nidx].glued_faces[opposite_face[face_index]] = false;
        voxels[nidx].glued_neighbors[opposite_face[face_index]] = -1;
    }
    voxel->glued_faces[face_index] = false;
    voxel->glued_neighbors[face_index] = -1;
    collisionTopologyDirty = true;
}
...
static void deactivate_fluid_cell(int voxel_idx) {
    ...
    int tgx = (int)floorf(voxel->pos.x / VOXEL_SIZE);
    int tgy = (int)floorf(voxel->pos.y / VOXEL_SIZE);
    int tgz = (int)floorf(voxel->pos.z / VOXEL_SIZE);
    int existing = table_get_static_only(tgx, tgy, tgz);
    if (existing >= 0 && existing != voxel_idx) {
        int best_x = tgx, best_y = tgy, best_z = tgz;
        float best_dist_sq = FLT_MAX;
        for (int dy = -1; dy <= 1; ++dy) {
            for (int dz = -1; dz <= 1; ++dz) {
                for (int dx = -1; dx <= 1; ++dx) {
                    if (dx == 0 && dy == 0 && dz == 0) continue;
                    if (table_get_static_only(tgx + dx, tgy + dy, tgz + dz) < 0) {
                        float cx = (tgx + dx + 0.5f) * VOXEL_SIZE;
                        float cy = (tgy + dy + 0.5f) * VOXEL_SIZE;
                        float cz = (tgz + dz + 0.5f) * VOXEL_SIZE;
                        float dsq = (cx - voxel->pos.x) * (cx - voxel->pos.x) +
                                    (cy - voxel->pos.y) * (cy - voxel->pos.y) +
                                    (cz - voxel->pos.z) * (cz - voxel->pos.z);
                        if (dsq < best_dist_sq) {
                            best_dist_sq = dsq;
                            best_x = tgx + dx;
                            best_y = tgy + dy;
    ...
    voxel->pos = (Vector3){
        ((float)tgx + 0.5f) * VOXEL_SIZE,
        ((float)tgy + 0.5f) * VOXEL_SIZE,
        ((float)tgz + 0.5f) * VOXEL_SIZE
    };
    ...
    for (int c = 0; c < VOXEL_CORNER_COUNT; ++c) {
        if (voxel->particles[c]) {
            voxel->particles[c]->pos = (Vector3){
                voxel->pos.x + (float)corner_signs[c][0] * offset,
                voxel->pos.y + (float)corner_signs[c][1] * offset,
                voxel->pos.z + (float)corner_signs[c][2] * offset
            };
            voxel->particles[c]->predicted_pos = voxel->particles[c]->pos;
            voxel->particles[c]->prev_pos = voxel->particles[c]->pos;
            voxel->particles[c]->vel = (Vector3){ 0.0f, 0.0f, 0.0f };
            ...
            if (calm) {
                for (int c = 0; c < VOXEL_CORNER_COUNT; ++c) {
                    ...
                    if (p->pbf_density > 0.0f) {
                        ...
                        if (density_ratio < 0.35f || density_ratio > 1.35f) {
                            calm = false;
                            break;
                        }
                    }
...
static int compare_fluid_candidate_height(const void *a, const void *b) {
    int idx_a = *(const int *)a;
    int idx_b = *(const int *)b;
    float ya = voxels[idx_a].pos.y;
    float yb = voxels[idx_b].pos.y;
    if (ya < yb) return -1;
    if (ya > yb) return 1;
    return 0;
}
...
static bool deactivate_sleeping_voxels(void)
{
    bool changed = false;
    bool restored_static = false;
    int remaining_solid_budget = VOXEL_MAX_DEACTIVATIONS_PER_FRAME;
    int remaining_fluid_budget = VOXEL_MAX_DEACTIVATIONS_PER_FRAME;
    ...
    int idx = 0;
    while (idx < voxel_count && remaining_solid_budget > 0) {
        Voxel *voxel = &voxels[idx];
        if (!voxel_is_awake_dynamic(voxel) || voxel->type != 0 || voxel->isBullet) {
            ++idx;
            continue;
        ...
        if (debugLogVoxelDeactivation) {
            TraceLog(LOG_INFO,
                     "[Deactivate] %s cluster starting=%d count=%d remaining_solid_budget_before=%d",
                     near_original ? "restored near-origin" : "froze in place",
                     idx, cluster_count, remaining_solid_budget);
        }
        remaining_solid_budget -= cluster_count;
        if (remaining_solid_budget < 0) {
            remaining_solid_budget = 0;
        }
        changed = true;
        continue;
    }
    ...
    // Advance sleepFrames independently per dynamic fluid voxel
    for (int i = 0; i < voxel_count; ++i) {
        Voxel *voxel = &voxels[i];
    ...
    // Collect all fluid voxels eligible to deactivate
    int fluid_candidate_count = 0;
    for (int i = 0; i < voxel_count; ++i) {
        Voxel *voxel = &voxels[i];
        if (!voxel->simulate || !voxel_is_fluid(voxel)) {
            continue;
        }
        if (voxel->sleepFrames >= FLUID_DEACTIVATION_FRAMES) {
            glueClusterIndices[fluid_candidate_count++] = i;
        }
    }

    // Sort candidates bottom-up (lowest y first) so bottom fluid settles first,
    // providing static support for the layers above it.
    if (fluid_candidate_count > 1) {
        qsort(glueClusterIndices, fluid_candidate_count, sizeof(int), compare_fluid_candidate_height);
    }

    // Greedily deactivate supported candidates up to the fluid deactivation budget
    for (int c = 0; c < fluid_candidate_count && remaining_fluid_budget > 0; ++c) {
        int candidate_idx = glueClusterIndices[c];
        Voxel *member = &voxels[candidate_idx];
        if (!member->simulate || !voxel_is_fluid(member)) {
            continue;
        }
        // If there is an awake fluid voxel directly above, ensure it is calm too
        // so we don't freeze the bottom while a waterfall is crashing down into it.
        int gx = (int)floorf(member->pos.x / VOXEL_SIZE);
        int gy = (int)floorf(member->pos.y / VOXEL_SIZE);
        int gz = (int)floorf(member->pos.z / VOXEL_SIZE);
        int above_idx = table_get(gx, gy + 1, gz);
        if (above_idx >= 0 && above_idx < voxel_count) {
            Voxel *above = &voxels[above_idx];
            if (above->simulate && voxel_is_fluid(above) && !voxelCalmFlags[above_idx]) {
                continue;
            }
        }
        if (fluid_voxel_has_support(member)) {
            deactivate_fluid_cell(candidate_idx);
            remaining_fluid_budget--;
            changed = true;
            fluid_deactivated = true;
        }
    }

    if (restored_static || fluid_deactivated) {
        rebuild_voxel_hash();
        rebuild_all_voxel_surfaces();
        rebuild_glue_constraints();
        refresh_static_voxel_beliefs();
...
static void handle_pbd_projectile_hits(void)
{
    ...
    while (i < voxel_count) {
        Voxel *v = &voxels[i];
        if (!v->simulate || v->sleeping || voxel_is_fluid(v)) {
            ++i;
            continue;
        }
...
static void evaluate_voxel_fracture_range(int start, int end, int worker_id, void *user) {
    (void)worker_id;
    (void)user;
    for (int i = start; i < end; ++i) {
        int v_idx = active_voxels[i];
...
static inline bool is_static_solid_cell(int x, int y, int z) {
    int idx = table_get_static_only(x, y, z);
    if (idx < 0 || idx >= voxel_count) return false;
    return !voxel_is_fluid(&voxels[idx]);
}
        ...
        for (int z = voxel->min_gz; z <= voxel->max_gz; ++z) {
            for (int y = voxel->min_gy; y <= voxel->max_gy; ++y) {
                for (int x = voxel->min_gx; x <= voxel->max_gx; ++x) {
                    ...
                    if (!is_static_solid_cell(x + 1, y, z) &&
                    ...
                    if (!is_static_solid_cell(x, y + 1, z) &&
                    ...
                    if (!is_static_solid_cell(x, y, z + 1) &&
...
static void solve_static_collision_particle(Particle *p, float half_player) {
    ...
    for (int s = 0; s < static_count; ++s) {
        ...
        if (voxel_is_fluid(static_voxel)) {
            bool energetic = (p->material == PARTICLE_MATERIAL_FLUID)
                ? (v_length(p->vel) >= FLUID_DEACTIVATION_VELOCITY_THRESHOLD * 1.5f)
                : (v_length(p->vel) >= VOXEL_DEACTIVATION_VELOCITY_THRESHOLD);
            if (energetic) {
...
static void update_voxel_velocity_range(int start, int end, int worker_id, void *user) {
    ...
    for (int i = start; i < end; ++i) {
        int v_idx = active_voxels[i];
        Voxel *voxel = &voxels[v_idx];
        if ((voxel->type != VOXEL_TYPE_SOLID && !voxel_is_fluid(voxel)) || voxel->isBullet) {
            continue;
        }
...
static void simulate_voxel_pbd_cpu_steps(float sub_dt, int substeps) {
    ...
    for (int step = 0; step < substeps; ++step) {
        ...
        for (int it = 0; it < 1; ++it) {
            ...
            int snapshot_count = collision_particle_count + active_fluid_particle_count;
            if (snapshot_count > MAX_PARTICLES) {
                snapshot_count = MAX_PARTICLES;
            }
            if (collision_particle_count > 0) {
                int copy_solid = collision_particle_count;
                if (copy_solid > snapshot_count) copy_solid = snapshot_count;
                memcpy(particle_snapshot, collision_particles,
                       (size_t)copy_solid * sizeof(particle_snapshot[0]));
            }
            if (active_fluid_particle_count > 0 && collision_particle_count < snapshot_count) {
                int copy_fluid = snapshot_count - collision_particle_count;
                memcpy(particle_snapshot + collision_particle_count, fluid_particles,
                       (size_t)copy_fluid * sizeof(particle_snapshot[0]));
            }
...
static float melee_anim_progress(const Player *p, float now) {
    if (!p->meleeSwingActive) {
        return -1.0f;
    }
    float t = (now - p->meleeSwingStartTime) / MELEE_ANIM_DURATION_SECONDS;
    return saturatef(t);
}
...
static float melee_reach_fraction(float progress) {
    ...
    if (progress <= MELEE_ACTIVE_START_NORM) {
        float windup_t = progress / fmaxf(MELEE_ACTIVE_START_NORM, 1e-4f);
        return MELEE_BASE_REACH_FRAC * smoothstepf(windup_t);
    }
    if (progress <= MELEE_REACH_PEAK_NORM) {
        float extend_t = (progress - MELEE_ACTIVE_START_NORM) /
                         fmaxf(MELEE_REACH_PEAK_NORM - MELEE_ACTIVE_START_NORM, 1e-4f);
        float eased = smoothstepf(extend_t);
        return mixf(MELEE_BASE_REACH_FRAC, 1.0f, eased);
    }
    float retract_t = (progress - MELEE_REACH_PEAK_NORM) /
                      fmaxf(1.0f - MELEE_REACH_PEAK_NORM, 1e-4f);
    float eased = smoothstepf(retract_t);
    return mixf(1.0f, MELEE_BASE_REACH_FRAC * 0.35f, eased);
}

static bool melee_is_active_window(float progress) {
...
static void draw_melee_arm_world(int playerIdx) {
    ...
    float now = (float)GetTime();
    float raw_progress = (now - p->meleeSwingStartTime) / MELEE_ANIM_DURATION_SECONDS;
    if (raw_progress < 0.0f || raw_progress > 1.0f) {
        return;
    }
...
static void perform_melee(int idx) {
    ...
    p->meleeSwingStartTime = now;
}

static void update_melee_swings(void) {
    float now = (float)GetTime();
    for (int i = 0; i < activePlayers; ++i) {
        Player *p = &players[i];
        if (!p->meleeSwingActive) {
        ...
        float progress = melee_anim_progress(p, now);
        ...
        if (melee_hit_voxels(p, start, dir, reach)) {
            p->meleeSwingHitApplied = true;
        }
    }
}
...
static NetInputCommand net_sample_local_input(int local_index, int player_slot) {
    ...
    if (use_gamepad) {
        ...
        if (IsGamepadButtonPressed(local_index, GAMEPAD_BUTTON_RIGHT_FACE_DOWN)) command.pressed |= NET_INPUT_MELEE;
            ...
            case GAME_STATE_MENU: {
                ...
                BeginDrawing();
                    ...
                    if (!menuSetupSubmenuOpen) {
                        ...
                        // LAN text
                        const char *lan_text = TextFormat("H: Host | J: Join localhost | LAN local screens: %d (-/+)", netRequestedLocalPlayers);
                        DrawText(lan_text, SCREEN_WIDTH / 2 - MeasureText(lan_text, 18) / 2, boxY + 242, 18, DARKGRAY);
                        ...
                        // ==========================================
                        // SUB-MENU: FIREFIGHT & WORLD SETUP (1040 x 710)
                        // ==========================================
                        DrawRectangle(boxX, boxY, boxW, boxH, Fade(RAYWHITE, 0.92f));
                        DrawRectangleLines(boxX, boxY, boxW, boxH, DARKGRAY);
                        DrawRectangleLines(boxX + 1, boxY + 1, boxW - 2, boxH - 2, DARKGRAY);

                        DrawText("FIREFIGHT & WORLD GENERATOR SETUP", boxX + 28, boxY + 18, 26, (Color){ 25, 30, 45, 255 });
                        Rectangle backRec = { (float)(boxX + boxW - 228), (float)(boxY + 16), 200.0f, 34.0f };
                        if (DrawMenuButton(backRec, "< Back to Menu (ESC)", 14, false, (Color){ 80, 90, 110, 255 }, (Color){ 230, 235, 242, 240 }, BLACK)) {
                            menuSetupSubmenuOpen = false;
                            menuEditingSeed = false;
                        }

                        const char *subHeader = "Select any architectural world archetype, customize seeds live, and configure Firefight:";
                        ...
                        const char *modeText = (gameMode == GAME_MODE_FIREFIGHT) ? "Mode: FIREFIGHT [Waves]" : "Mode: DEATHMATCH [PvP]";
                        ...
                        DrawText("- [ESC / F]: Back to Main Menu            |  [ENTER]: Start Selected Match", boxX + 42, boxY + 620, 13, DARKBLUE);
                        DrawText("- [1 - 8]: Switch World Type instantly     |  [W / Q]: Cycle Next / Previous World", boxX + 42, boxY + 640, 13, (Color){ 40, 40, 55, 255 });
                        DrawText("- [Left / Right]: Seed +/- 1               |  [E]: Type Custom Seed  |  [T / R]: Random Seed", boxX + 42, boxY + 660, 13, (Color){ 40, 40, 55, 255 });
                        DrawText("- Click Mode button to switch between FIREFIGHT (Waves) and DEATHMATCH (PvP)", boxX + 42, boxY + 678, 13, (Color){ 40, 40, 55, 255 });
                ...
                if (!menuSetupSubmenuOpen) {
                    ...
                    if (IsKeyPressed(KEY_Q)) {
                        currentWorldType = (WorldType)((currentWorldType + WORLD_TYPE_COUNT - 1) % WORLD_TYPE_COUNT);
                        ResetGame();
                    }
                    ...
                    // Inputs for Firefight & World Setup Sub-Menu
                    if (menuEditingSeed) {
        ...
        break;
            ...
            case GAME_STATE_SETTINGS:
                ...
                BeginDrawing();
                    ...
                    DrawText("Gamepad: Move LS | Look RS | Jump B | Shoot RT | Melee A | Build RB | Tether LT",
