...
static void greedy_group_test(void){reset_test();UnitVoxelBuffer b={0};GreedyCell cells[28];int n=0;
    ...
    assert(passive>=0&&mapped>=0);Particle *pp=greedyCoarse.samples[passive];Vector3 passive_goal=pp->predicted_pos;Vector3 control_before=greedyCoarse.sources[passive][0]->predicted_pos;pp->predicted_pos=v_add(pp->predicted_pos,(Vector3){3,2,1});greedy_refresh_predicted(false);near_vector(greedyCoarse.sources[passive][0]->predicted_pos,control_before,1e-7f);greedy_refresh_predicted(true);near_vector(pp->predicted_pos,passive_goal,1e-6f);assert(pp->sim_index<0);
    Particle *mp=greedyCoarse.samples[mapped];Vector3 source_before=greedyCoarse.sources[mapped][0]->predicted_pos;reset_particle_accumulators();assert(greedy_accumulate_mapped_correction(mp,(Vector3){.01f,0,0},1));apply_particle_accumulators();assert(v_length(v_sub(greedyCoarse.sources[mapped][0]->predicted_pos,source_before))>1e-7f);greedy_refresh_predicted(false);
    ...
    tetherTag[0]=1;assert(gpu_pack_world());assert(gpuPhysics.greedy_world&&gpuPhysics.dynamic_voxel_count==greedyCoarse.physics_shape_count);ptrdiff_t mapped_pool=mp-particles_pool;assert(mapped_pool>=0);int mapped_compact=gpuPhysics.pool_to_compact[mapped_pool];assert(mapped_compact>=0);GpuDependencyHeader packed=gpuPhysics.dependency_header_stage[mapped_compact];assert((packed.flags&(FPS_GPU_DEPENDENCY_MAPPED|FPS_GPU_DEPENDENCY_PARENT_USE))==(FPS_GPU_DEPENDENCY_MAPPED|FPS_GPU_DEPENDENCY_PARENT_USE));assert((int)packed.count==greedyCoarse.source_count[mapped]);for(uint32_t k=0;k<packed.count;k++){GpuDependencyEntry e=gpuPhysics.dependency_entry_stage[packed.offset+k];assert(e.source<(uint32_t)gpuPhysics.compact_count);assert(fabsf(e.weight-greedyCoarse.weights[mapped][k])<1e-7f);assert(fabsf(e.correction_weight-greedyCoarse.correction_weights[mapped][k])<1e-7f);}int tethered_sources=0;for(int i=0;i<gpuPhysics.compact_count;i++)tethered_sources+=gpuPhysics.tether_stage[i]==0;assert(tethered_sources>0);tetherTag[0]=0;
