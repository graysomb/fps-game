...
static void greedy_lifecycle_test(void){reset_test();static UnitVoxelBuffer b;memset(&b,0,sizeof(b));GreedyCell cells[64];int n=0;for(int z=0;z<4;z++)for(int y=5;y<9;y++)for(int x=0;x<4;x++){int k=n++;int v=debug_add_cell(x,y,z,false,true,WHITE,73);assert(v>=0);b.voxels[k]=(UnitVoxelSeed){.gx=x,.gy=y,.gz=z,.type=0,.voxelIndex=v};cells[k]=(GreedyCell){x,y,z,0,k};}b.count=n;debug_rebuild_world_state();assert(greedy_share_range(0,n));GreedyCubeCover cover={0};assert(greedy_cube_cover_build(cells,n,&cover));assert(greedy_coarse_bind(&cover,&b,0));greedy_cube_cover_free(&cover);rebuild_all_voxel_surfaces();greedy_build_collision_samples();rebuild_glue_constraints();int groups=greedyCoarse.group_count,children=greedyCoarse.child_count;assert(groups==1&&children==64);int wake_child=greedyCoarse.children[0];assert(freeze_dynamic_cluster_in_place(greedyCoarse.children,greedyCoarse.child_count));greedyCoarse.shapes[0].wake_timer=-123;assert(wake_sleeping_cluster_for_gameplay(wake_child));assert(greedyCoarse.active&&greedyCoarse.shapes[0].wake_timer==COARSENING_WAKE_FRAMES);Particle *points[256];Vector3 poses[256];int point_count=active_particle_count;assert(point_count<=256);float cs=cosf(.37f),sn=sinf(.37f);for(int i=0;i<point_count;i++){Particle *p=points[i]=active_particles[i];Vector3 q=p->pos,r={cs*q.x+sn*q.z+.31f,q.y+.17f,-sn*q.x+cs*q.z-.22f};p->pos=p->prev_pos=p->predicted_pos=poses[i]=r;}assert(greedy_rebuild_current());assert(greedyCoarse.active&&greedyCoarse.group_count==groups&&greedyCoarse.child_count==children);for(int i=0;i<point_count;i++)near_vector(points[i]->pos,poses[i],2e-5f);
    int before_sim=sim_particle_count;Particle *corner=greedyCoarse.shapes[0].particles[7];corner->predicted_pos.x+=VOXEL_SIZE*2.0f;assert(greedy_process_pending_fracture());if(!greedyCoarse.active)fprintf(stderr,"local fracture unexpectedly refined all %d children\n",children);assert(greedyCoarse.active&&greedyCoarse.child_count>0&&greedyCoarse.child_count<children);assert(sim_particle_count>before_sim);int fine_children=0;bool broken=false;for(int v=0;v<voxel_count;v++){assert(!voxels[v].sleeping);fine_children+=greedyCoarse.voxel_group[v]<0;for(int f=0;f<6;f++)broken|=!voxels[v].glued_faces[f];}assert(fine_children>0&&broken);for(int i=0;i<sim_particle_count;i++)assert(sim_particles[i]->sim_index==i);}
static void greedy_repeated_activation_test(void){reset_test();static UnitVoxelBuffer a,b;memset(&a,0,sizeof(a));memset(&b,0,sizeof(b));for(int k=0;k<8;k++){int x=k&1,y=5+((k>>1)&1),z=(k>>2)&1;int v=debug_add_cell(x,y,z,true,false,WHITE,74);assert(v>=0);assert(unit_voxel_buffer_push(&a,x,y,z,WHITE,0,true,v,74,-1));}debug_rebuild_world_state();assert(emit_greedy_dynamic_from_buffer(&a));assert(greedyCoarse.child_count==8);uint64_t retained=voxels[greedyCoarse.children[0]].identity;int extra=debug_add_cell(2,5,0,true,false,BLUE,74);assert(extra>=0);assert(unit_voxel_buffer_push(&b,2,5,0,BLUE,0,true,extra,74,-1));debug_rebuild_world_state();assert(emit_greedy_dynamic_from_buffer(&b));assert(greedyCoarse.active&&greedyCoarse.child_count==9);int retained_index=find_voxel_by_identity(retained);assert(retained_index>=0);bool retained_child=false;for(int i=0;i<greedyCoarse.child_count;i++)retained_child|=greedyCoarse.children[i]==retained_index;assert(retained_child);remove_voxel_index(retained_index);assert(!greedyCoarse.active&&voxel_count==8);}
static void assert_greedy_attachment_state(int step) {
    assert(greedyCoarse.active);
    float error=0;
